...
env = square_maze(maze_matrix, maze_code)
...
for ep in range(start_params["n_ep"]):
    ...
    reward_pos = -1 #outside the environment
    ...
    step = 0
    for steppy in range(start_params["n_steps"]):
        ...
        if steppy >= start_params["n_steps"]-1:
          network_state = "SWR"
        ...
        if network_state == "Theta":
            ...
            for pred in range(start_params["SWR_compression_factor"]):
                ...
                seq_loss.append(batch_loss_SWR)
...
os.chdir('./C2_produced_data')

id_label = "Corridor_randomwalk_" + str(start_params["n_ep"])
